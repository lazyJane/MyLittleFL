...
def main():
    ...
    n_tasks = int(len(os.listdir(RAW_DATA_PATH)) * args.s_frac) # 用户数量 * 比率
    file_names_list = os.listdir(RAW_DATA_PATH)
    rng.shuffle(file_names_list)

    public_data, public_targets = torch.load(os.path.join(RAW_DATA_PATH, file_names_list[0])) # 301
    public_data_size = public_data.shape[0]
    print("public_data_size",public_data_size) #271
    #print(public_data, public_targets)
    input()

    os.makedirs(os.path.join(TARGET_PATH), exist_ok=True)
    torch.save((public_data, public_targets), os.path.join(TARGET_PATH, "public.pt"))

    file_names_list = file_names_list[1:n_tasks]
    rng.shuffle(file_names_list)

    os.makedirs(os.path.join(TARGET_PATH, "participate"), exist_ok=True)
    os.makedirs(os.path.join(TARGET_PATH, "unseen"), exist_ok=True)
    
    print("generating data..")
    sum_data = 0
    data_users = []
    user_targets_num = np.zeros([n_tasks, 62])
    for idx, file_name in enumerate(tqdm(file_names_list)): 
        if idx >= 0 and idx < int(args.unseen_tasks_frac * n_tasks):
            mode = "participate"
        else:
            mode = "unseen"

        data, targets = torch.load(os.path.join(RAW_DATA_PATH, file_name)) # torch.Size([183, 28, 28])
        #print(targets)
        #input()
        #user_targets_num[idx][] += cnt 
    ...
    print(sum_data)
    for idx, file_name in enumerate(tqdm(file_names_list)):  
        print("user-ratio", idx, ":",float(data_users[idx].shape[0]),float(data_users[idx].shape[0]/sum_data))
