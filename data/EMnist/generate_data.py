...
def main():
    ...
    dataset =\
        ConcatDataset([
            EMNIST(root=RAW_DATA_PATH, split="balanced", download=True, train=True, transform=transform),
            EMNIST(root=RAW_DATA_PATH, split="balanced", download=False, train=False, transform=transform)
        ]) # len(dataset) 814255   letters #145600
    a= EMNIST(root=RAW_DATA_PATH, split="balanced", download=True, train=True, transform=transform)
    print(a.classes)
    input()
    ...
    for key, value in label2index.items():
        public_indices += value[0:1] # public从每类中取的数据量
        #print(len(label2index[key]))
        label2index[key] = value[1:]
